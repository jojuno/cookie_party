...
function mortimer_epicenter:GetCastPoint()
    ...
    --play sound
    self:GetCaster():EmitSound("morty_roar")
	return 0
end

--------------------------------------------------------------------------------
-- Ability Cast Filter
function mortimer_epicenter:CastFilterResultTarget( hTarget )
...
function mortimer_epicenter:GetCustomCastErrorTarget( hTarget )
	--[[if IsServer() and hTarget:IsChanneling() then
...
--------------------------------------------------------------------------------
-- Ability Phase Start
function mortimer_epicenter:OnAbilityPhaseInterrupted()

end
function mortimer_epicenter:OnAbilityPhaseStart()
    print("[mortimer_epicenter:OnAbilityPhaseStart()] called")
...
function mortimer_epicenter:OnSpellStart()
    print("[mortimer_epicenter:OnSpellStart()] called")
	-- unit identifier
    ...
    Timers:CreateTimer(1, function()
        if self.jump == 3 then
            ...
            local callback = function()
                print("[mortimer_epicenter:OnSpellStart()] callback called")
                -- precache damage
                local damageTable = {
                    -- victim = target,
                ...
                for _,enemy in pairs(enemies) do
                    print("[mortimer_epicenter:OnSpellStart()] enemy found")
                    -- apply damage
