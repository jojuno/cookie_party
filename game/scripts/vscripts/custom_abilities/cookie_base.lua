...
function cookie_base:CastFilterResultTarget( hTarget )
    ...
    if self:GetCaster():HasAbility("cookie_raisin_firesnap") or self:GetCaster():HasAbility("cookie_bakers_dozen") then
        nResult = UnitFilter(
            hTarget,
...
function cookie_base:OnProjectileHit( target, location )
    ...
    if self:GetCaster():HasAbility("cookie_bakers_dozen") then
        ...
        if target:GetTeam() == self:GetCaster():GetTeam() then
            --distance_secondary = self:GetCaster():FindAbilityByName("cookie_bakers_dozen"):GetSpecialValueFor( "horizontal_jump_distance_ally" )
            distance_secondary = 450
        else
            --distance_secondary = self:GetCaster():FindAbilityByName("cookie_bakers_dozen"):GetSpecialValueFor( "horizontal_jump_distance_enemy" )
    ...
    if target ~= self.target then
        print("projectile hit for secondary target")
        ...
        local callback = function()
            ...
            self:RaisinFiresnapEffect(self:GetCaster(), target)
    ...
    --primary target
        ...
        if target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
            ...
            self:BakersDozenEffect(self:GetCaster(), target)

            -- cookie_oven effect
            if self:GetCaster():HasAbility("cookie_oven") then
                self:OvenEffect(self:GetCaster(), target)
            end
...
function cookie_base:RaisinFiresnapEffect(caster, target)
    ...
    if caster:HasAbility("cookie_raisin_firesnap") then
        print("[cookie_base:OnProjectileHit( target, location )] caster has cookie_raisin_firesnap")
...
function cookie_base:OvenThinker(oven)
    ...
    Timers:CreateTimer(function()
        ...
        counter = counter + 1
        if counter == 5 or oven:IsAlive() == false then
            oven:RemoveSelf()
        else
            return 2.0
            --return nil
