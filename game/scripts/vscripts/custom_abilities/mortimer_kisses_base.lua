...
function mortimer_kisses_modifier:OnCreated( kv )
	...
	self:GetCaster().teleportCast = false

	--table of shadowraze units
	if self:GetAbility().addedEffect == 5 then
	...
	local interval = self:GetAbility():GetDuration() / projectile_count + 0.01 -- so it only has 8 projectiles instead of 9
	...
	local effectName
	if self:GetCaster():HasAbility("mortimer_kisses_added_effect") then
		--based on effect id, change it
		if self:GetAbility().addedEffect == 1 then
			--blue
			effectName = "particles/snapfire_lizard_blobs_arced_ice.vpcf"
		elseif self:GetAbility().addedEffect == 2 then
			--green
			effectName = "particles/snapfire_lizard_blobs_arced_green.vpcf"
		elseif self:GetAbility().addedEffect == 3 then
			--red
			effectName = "particles/snapfire_lizard_blobs_arced_red.vpcf"
		elseif self:GetAbility().addedEffect == 4 then
			--purple
			effectName = "particles/snapfire_lizard_blobs_arced_purple.vpcf"
		elseif self:GetAbility().addedEffect == 5 then
			--black
	...
	if self:GetCaster():HasAbility("mortimer_kisses_teleport") then
		...
		local teleport_ability = self:GetCaster():FindAbilityByName("mortimer_kisses_teleport")
		if not teleport_ability:IsTrained() then
			teleport_ability:SetLevel(1)
		end
		self:GetCaster():SwapAbilities("mortimer_kisses_teleport", "mortimer_kisses_base", true, true)
		--self:GetCaster():FindAbilityByName("mortimer_kisses_teleport"):SetAbilityIndex(5)
...
function mortimer_kisses_modifier:OnIntervalThink()
	...
	if self:GetCaster():IsStunned() or self:GetCaster():IsSilenced() then
		...
		if self:GetCaster():HasAbility("mortimer_kisses_make_it_rain") then
			local blobs = 0
			Timers:CreateTimer("extra_blobs", {
				...
				callback = function()
					...
					if blobs == self:GetCaster():FindAbilityByName("mortimer_kisses_make_it_rain"):GetSpecialValueFor("extra_blobs") then
						return nil
					else
						return 0.1
					end
...
function mortimer_kisses_modifier:CreateBlob(kv)
	...
	if self:GetCaster():HasAbility("mortimer_kisses_teleport") then
		self.remnants[self.blob_count] = CreateUnitByName("dummy", target_pos, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeam())
	...
	--shadow raze upgrade
	if self:GetAbility().addedEffect == 5 then
		if self:GetCaster():HasModifier("mortimer_kisses_modifier") then
...
function mortimer_kisses_modifier:CreateBlobExtra(kv)
	...
	local target_pos = Vector(self:GetCaster():GetAbsOrigin().x + math.random(-1500, 1500),
		self:GetCaster():GetAbsOrigin().y + math.random(-1500, 1500),
		0)
	...
	local travel_time = self.travel_time
	travel_time = ((target_pos - self:GetCaster():GetAbsOrigin()):Length2D()-self.min_range)/self.range * self.travel_range + self.min_travel
...
function mortimer_kisses_modifier:ShadowRazeUpgrade(target_pos, travel_time)
	...
	local shadowrazeUnit = CreateUnitByName("dummy", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeam())
